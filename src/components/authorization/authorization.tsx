...
const Authorization = ({panel, changePanel, eye, changeEye, buttonClick}: AuthProps) => {
    ...
    return (
        <form className={styles.auth__container} style={{height: panel === 'auth' ? '773px' : '843px'}}>
            <span className={styles.container__text}>
                {panel === 'auth' ? 'Войти' : 'Регистрация'}
            </span>
            {panel === 'auth' ? <div style={{display:'none'}}/> :
            <div className={styles.container__input}>
                <span className={styles.input__text}>
                    Имя пользователя
                </span>
                <input type="text" value={userStore.userdata.name} onChange={changeName} className={styles.input} required/>
            </div> }
            <div className={styles.container__input}>
                <span className={styles.input__text}>
                    Email
                ...
                className={userStore.datavalid.validEmail ? styles.input : styles.input__invalid} 
            ...
            <div className={styles.container__input}>
                ...
                className={userStore.datavalid.validPassword ? styles.input : styles.input__invalid} 
                pattern='.{8,}'
                required/>
                <span className={styles.eye} onClick={changeEye}/>
            </div>
            <div className={styles.container__check} style={{display: panel === 'auth' ? 'flex' : 'none'}}>
            ...
            <button className={styles.container__button} onClick={buttonClick} style={{marginTop: panel === 'auth' ? '0px' : '36px'}}>
                {panel === 'auth' ? 'Войти в Accelerant' : 'Зарегистрироваться'}
            </button>
            <span className={styles.container__br}>
                или войти через
            </span>
            ...
            <span className={styles.container__footer}>
                Еще нет аккаунта? <span onClick={changePanel}>Зарегистрироваться</span>
            </span> : 
            <span className={styles.container__footer}>
