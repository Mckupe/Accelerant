...
const Сhangesoc = () => {
    ...
    async function delSoc(e: any) {
        ...
        if (!re.test(valueLink) || !re.test(valueToken)) {
            ...
            await axios({
                method: 'delete',
                url: 'http://localhost:5000/api/soc/deleteSoc',
            ...
            <div className={styles.main}>
                <div className={styles.logoandtext__block}>
                    ...
                    {socetArray.includes("vk") ?
                        <>
                            <button onClick={delSoc} className={styles.button}>Удалить</button>
                        </>
                        : <button onClick={() => modalStore.changeModalVk()} className={styles.button}>Подключить</button>
                    }
                </div>
                <div className={styles.logoandtext__block}>
